...
export function WellPlateBasic({ ...props }) {
    ...
    useEffect(() => {
        scanDispatch(setSelectedWells(plateState.selectedWells));
    }, [plateState.selectedWells]);

    useEffect(() => {
        scanDispatch(setWells(plateState.wells));
    }, [plateState.wells])

    return (
        <>
            <svg width="750" height="518" viewBox="0 0 750 518" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M732.93 0.73513H17.0703C8.04872 0.73513 0.735291 8.0469 0.735291 17.0664V500.604C0.735291 509.624 8.04872 516.936 17.0703 516.936H732.93C741.951 516.936 749.265 509.624 749.265 500.604V17.0664C749.265 8.0469 741.951 0.73513 732.93 0.73513Z" fill="white" stroke={error ? 'red' : '#E9ECEF'} />
                <path d="M44.4987 43.7041L44.5087 43.6945L44.5182 43.6845L58.2324 29.4463C61.3115 26.2493 65.5596 24.443 69.9988 24.443H687.257H713.715C719.806 24.443 724.744 29.38 724.744 35.4699V454.14V480.59C724.744 486.679 719.806 491.617 713.715 491.617H687.257H69.6215C65.3959 491.617 61.3346 489.979 58.2907 487.048L43.7971 473.096L43.7775 473.077L29.8213 458.586C26.8903 455.542 25.2528 451.483 25.2528 447.257V69.1789C25.2528 64.7406 27.0594 60.4936 30.2572 57.4151L44.4987 43.7041Z" fill="white" stroke={error ? 'red' : '#E9ECEF'} />
                ...
                <foreignObject x="25" y="25" width="700" height="466">
                    <div className={`plate ${classes.plate}`}>
                        ...
                        <Group grow gap={8} direction="column" className={classes.rows}>
                            {rows.map((row) => (
                                <div className="row-header" key={row}>
                                    {row}
                                </div>
                            ))}
                        </Group>
